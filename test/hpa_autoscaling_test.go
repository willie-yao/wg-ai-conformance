...
const (
	...
	hpaPolicyUpdateTimeout    = time.Minute
...
func TestAcceleratorHorizontalPodAutoscaling(t *testing.T) {
	...
	pods, err = waitForHPAScaleUp(ctx, clientset, namespace, workloadName, selector, mode, cfg, hpaScaleTimeout)
	if err != nil {
		t.Fatal(err)
	}
	if err := verifyDRAAllocations(ctx, clientset, pods, mode); err != nil {
	...
	if err := enableHPAScaleDown(ctx, clientset, namespace, workloadName); err != nil {
		t.Fatalf("HPA FAILURE: Could not enable scale-down: %v", err)
	}
...
func buildAcceleratorHPA(namespace, name string, objectLabels map[string]string, metricName string) *autoscalingv2.HorizontalPodAutoscaler {
	...
	// Keep both Pods until the test has observed both metrics at zero.
	scaleDownPolicy := autoscalingv2.DisabledPolicySelect
	return &autoscalingv2.HorizontalPodAutoscaler{
		...
		Spec: autoscalingv2.HorizontalPodAutoscalerSpec{
			...
			Behavior: &autoscalingv2.HorizontalPodAutoscalerBehavior{ScaleDown: &autoscalingv2.HPAScalingRules{
				...
				SelectPolicy:               &scaleDownPolicy,
			}},
		},
	}
}

func enableHPAScaleDown(ctx context.Context, c kubernetes.Interface, namespace, name string) error {
	requestCtx, cancel := context.WithTimeout(ctx, hpaPolicyUpdateTimeout)
	defer cancel()
	patch := []byte(`{"spec":{"behavior":{"scaleDown":{"selectPolicy":"Max"}}}}`)
	_, err := c.AutoscalingV2().HorizontalPodAutoscalers(namespace).Patch(requestCtx, name, types.MergePatchType, patch, metav1.PatchOptions{})
	return err
}
...
func parsePodMetrics(data []byte, metricName string) (map[string]resource.Quantity, error) {
	...
	for _, item := range list.Items {
		...
		if item.Value == nil {
			return nil, fmt.Errorf("missing custom metric value for Pod %s", item.DescribedObject.Name)
		}
...
func waitForPodMetrics(ctx context.Context, c kubernetes.Interface, namespace, metricName, selector string, timeout time.Duration, matches func(map[string]resource.Quantity) bool) (map[string]resource.Quantity, error) {
	...
	if err != nil {
		return latest, fmt.Errorf("waiting for Pod metrics failed (limit %s); latest=%s%s: %w", timeout, formatMetricValues(latest), lastAPIErrorSuffix(lastErr), err)
	}
	return latest, nil
}
...
func waitForHPAReplicas(ctx context.Context, c kubernetes.Interface, namespace, name string, want int32, timeout time.Duration) (*autoscalingv2.HorizontalPodAutoscaler, error) {
	...
	if err != nil {
		return latest, fmt.Errorf("waiting for desiredReplicas=%d failed (limit %s); latest=%s%s: %w", want, timeout, formatHPAStatus(latest), lastAPIErrorSuffix(lastErr), err)
	}
	return latest, nil
}
...
func waitForDeploymentReplicas(ctx context.Context, c kubernetes.Interface, namespace, name, selector string, want int32, timeout time.Duration) ([]corev1.Pod, error) {
	...
	if err != nil {
		return pods, fmt.Errorf("waiting for %d Ready replicas failed (limit %s); deployment=%s pods=%s%s: %w", want, timeout, formatDeploymentStatus(deployment), formatPodStatuses(pods), lastAPIErrorSuffix(lastErr), err)
	}
	return readyPods(pods), nil
}

func waitForHPAScaleUp(ctx context.Context, c kubernetes.Interface, namespace, name, selector, mode string, cfg AcceleratorConfig, timeout time.Duration) ([]corev1.Pod, error) {
	pods, err := waitForDeploymentReplicas(ctx, c, namespace, name, selector, hpaMaximumReplicas, timeout)
	if err == nil {
		return pods, nil
	}
	diagnostic, capacityShortfall, diagnosticErr := classifyAcceleratorCapacityShortfall(ctx, c, namespace, selector, mode, cfg)
	if capacityShortfall {
		return nil, fmt.Errorf("ENVIRONMENT ERROR: HPA requested two replicas; a fresh observation found an accelerator-capacity scheduling shortfall: %s; readiness wait failed: %w", diagnostic, err)
	}
	if diagnosticErr != nil {
		err = errors.Join(err, fmt.Errorf("capacity diagnosis unavailable: %w", diagnosticErr))
	}
	return nil, fmt.Errorf("ACCELERATOR WORKLOAD FAILURE: HPA requested two replicas, but the second accelerator-backed Pod did not become Ready: %w", err)
}
...
func classifyAcceleratorCapacityShortfall(ctx context.Context, c kubernetes.Interface, namespace, selector, mode string, cfg AcceleratorConfig) (string, bool, error) {
	checkCtx, cancel := context.WithTimeout(ctx, hpaCapacityCheckTimeout)
	defer cancel()
	list, err := c.CoreV1().Pods(namespace).List(checkCtx, metav1.ListOptions{LabelSelector: selector})
	if err != nil {
		return "", false, fmt.Errorf("refresh workload Pods: %w", err)
	}
	pods := list.Items
	...
	claimNames, err := podGeneratedClaims(checkCtx, c, pending.Namespace, pending.Name, pending)
	if err != nil {
		return "", false, fmt.Errorf("find generated ResourceClaims for Pod %s: %w", pending.Name, err)
	}
	if len(claimNames) != 1 {
		return "", false, nil
	}
	claim, err := c.ResourceV1().ResourceClaims(pending.Namespace).Get(checkCtx, claimNames[0], metav1.GetOptions{})
	if err != nil {
		return "", false, fmt.Errorf("get ResourceClaim %s: %w", claimNames[0], err)
	}
	if claim.Status.Allocation != nil {
		return "", false, nil
	}
	return fmt.Sprintf("%s; generated ResourceClaim %s is unallocated", diagnostic, claim.Name), true, nil
}
...
func cleanupHPAWorkload(ctx context.Context, c kubernetes.Interface, namespace, name, selector, claimTemplateName, mode string) error {
	...
	if err := waitForHPAWorkloadGone(cleanupCtx, c, namespace, name, selector); err != nil {
...
func waitForHPAWorkloadGone(ctx context.Context, c kubernetes.Interface, namespace, name, selector string) error {
	var lastErr error
	...
	if err != nil {
		return fmt.Errorf("wait for Deployment %s and its Pods to be deleted%s: %w", name, lastAPIErrorSuffix(lastErr), err)
	}
	return nil
}
